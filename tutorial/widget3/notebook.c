                     ...
                     gchar *hint, gpointer user_data)
{
    ...
    gsize length;
    char *filename;
    // tv, tb -> scr, label -> notebook -> notebookpage -> win
    ...
    for (int i = 0; i < n_files; i++) {
        if (g_file_load_contents(files[i], NULL, &contents, &length, NULL, NULL)) {
            ...
            gtk_text_view_set_editable(GTK_TEXT_VIEW(tv), FALSE); // edit
            ...
            if ((filename = g_file_get_basename(files[i])) != NULL) {
                lab = gtk_label_new(filename); // is label not button
                g_free(filename);
            }
            else
                lab = gtk_label_new("");
    
            gtk_notebook_append_page(GTK_NOTEBOOK(nb), scr, lab); // add page, one file is a page
